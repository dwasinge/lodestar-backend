...
import com.redhat.labs.lodestar.model.filter.EngagementStatus;
...
public class MongoHelper {
    ...
    static Bson findByStatus(Optional<String> status, Optional<String> today) {
        ...
        String engagementStatus = status.orElse(EngagementStatus.UPCOMING.name());
        String localDate = today.orElse(LocalDate.now(ZoneId.of("Z")).toString());

        Bson bson;
        if (EngagementStatus.ACTIVE.name().equalsIgnoreCase(engagementStatus)) {
            bson = findActiveEngagements(localDate);
        } else if (EngagementStatus.TERMINATING.name().equalsIgnoreCase(engagementStatus)) {
            bson = findTerminatingEngagements(localDate);
        } else if (EngagementStatus.PAST.name().equalsIgnoreCase(engagementStatus)) {
